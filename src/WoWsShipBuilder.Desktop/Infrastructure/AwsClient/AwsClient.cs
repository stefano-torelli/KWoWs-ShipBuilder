...
public class AwsClient(IDataService dataService, IAppDataService appDataService, ILogger<AwsClient> logger, HttpMessageHandler? handler = null) : ClientBase(dataService, appDataService), IDesktopAwsClient
{
    private const string Host = "https://d2nzlaerr9l5k3.cloudfront.net";

    private readonly ILogger<AwsClient> logger = logger;

    protected override HttpClient Client { get; } = new(new RetryHttpHandler(handler ?? new HttpClientHandler()));
    ...
    public async Task DownloadImages(IFileSystem fileSystem, string? fileName = null)
    {
        ...
        var zipName = fileName ?? "ship";
        var zipUrl = @$"{Host}/images/ship/{zipName}.zip";
        var localFolder = "Ships";

        var directoryPath = fileSystem.Path.Combine(this.AppDataService.AppDataImageDirectory, localFolder);
        ...
        var zipPath = fileSystem.Path.Combine(directoryPath, $"{zipName}.zip");
        try
        {
        ...
        {
            this.logger.LogWarning(e, "Failed to download images from uri {ZipUrl}", zipUrl);
        }
    }

    public async Task<VersionInfo> DownloadVersionInfo(ServerType serverType)
    {
        var url = @$"{Host}/api/{serverType.StringName()}/VersionInfo.json";
    ...
    public async Task DownloadFiles(ServerType serverType, List<(string, string)> relativeFilePaths, IProgress<int>? downloadProgress = null)
    {
        ...
        var baseUrl = @$"{Host}/api/{serverType.StringName()}/";
        ...
        {
            var localFileName = this.DataService.CombinePaths(this.AppDataService.GetDataPath(serverType), category, fileName);
            ...
            var task = Task.Run(async () =>
            {
                ...
                {
                    this.logger.LogWarning(e, "Encountered an exception while downloading a file with uri {Uri} and filename {LocalFileName}", uri, localFileName);
                }
