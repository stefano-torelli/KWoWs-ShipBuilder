...
public static class Helpers
{
    public static string GetIconFromClass(ShipClass shipClass, ShipCategory category)
    {
        ...
        var fill = ClassToPathHelper.GetColorFromCategory(category, false)[3..];
    ...
    {
        var imgName = File.Exists(Path.Combine(environment.ContentRootPath, "wwwroot", "assets", "nation_flags", $"flag_{shipIndex}.png")) ? shipIndex : shipNation.ShipNationToString();
    ...
    public static List<ShipUpgrade> GetShipConfigurationFromBuild(IEnumerable<string> storedData, IEnumerable<ShipUpgrade> upgrades)
    {
        ...
        foreach (var upgradeList in shipUpgrades)
        {
